...
export const VenueTickets = ({ tickets, eventName, searchQuery, accentColor = "emerald" }) => {
  ...
  return (
    <section id="tickets" className="py-16 bg-white" data-testid="venue-tickets">
      <div className="max-w-6xl mx-auto px-4">
        <div className="flex items-center justify-between mb-8">
          <div>
            <h2 className="text-2xl sm:text-3xl font-bold text-slate-900">{eventName} Tickets</h2>
        ...
        <div className="grid gap-3">
          {tickets.map((t, i) => (
            <motion.div key={i} variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }} custom={i}
              className={`flex flex-col md:flex-row md:items-center justify-between bg-white border border-slate-100 ${c.hover} rounded-2xl p-5 transition-all group`}>
        ...
        <div className="mt-6 p-4 bg-slate-50 rounded-xl border border-slate-100 flex flex-wrap items-center justify-center gap-6 text-xs text-slate-500">
...
export const TrustSection = ({ reviewCount = 4850, rating = 4.8 }) => (
  <section className="py-12 bg-slate-50 border-y border-slate-100" data-testid="trust-section">
    <div className="max-w-6xl mx-auto px-4">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-6 text-center">
        <div>
          <div className="text-3xl font-black text-slate-900">{rating}</div>
...
export const RelatedEvents = ({ events, accentColor = "emerald" }) => {
  ...
  return (
    <section className="py-12 bg-white" data-testid="related-events">
      <div className="max-w-6xl mx-auto px-4">
        <h2 className="text-2xl font-bold text-slate-900 mb-6">Available Events</h2>
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {events.slice(0, 6).map((e, i) => (
            <motion.div key={i} variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }} custom={i}>
              <Link to={`/events/${e.event_id}`} className="block bg-white border border-slate-100 hover:border-slate-200 rounded-xl p-5 transition-all hover:shadow-md">
                <div className="flex items-start justify-between mb-3">
                  <div className="flex-1 min-w-0">
                    <h3 className="font-bold text-slate-900 text-sm truncate">{e.title}</h3>
