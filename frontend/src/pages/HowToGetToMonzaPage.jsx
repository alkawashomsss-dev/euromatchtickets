...
const HowToGetToMonzaPage = () => {
  ...
  return (
    <div className="min-h-screen bg-[hsl(210,20%,98%)]" data-testid="monza-travel-page">
      ...
      <section className="py-16">
        <div className="max-w-4xl mx-auto px-4">
          ...
          <h1 className="text-3xl md:text-4xl font-black text-slate-900 mb-4">How to Get to Monza F1 Grand Prix 2026</h1>
          <p className="text-lg text-slate-600 mb-10">Everything you need to know about travelling to the Autodromo Nazionale di Monza - from Milan airports, city center, and beyond.</p>

          {/* Transport Options */}
          <div className="space-y-6">
            {routes.map((r, i) => (
              <div key={i} className={`bg-white rounded-2xl border p-6 ${r.recommended ? 'border-emerald-300 ring-2 ring-emerald-50' : 'border-slate-200'}`} data-testid={`route-${i}`}>
                ...
                <div className="flex items-start gap-4">
                  <div className="w-10 h-10 bg-slate-100 rounded-xl flex items-center justify-center flex-shrink-0"><r.icon className="w-5 h-5 text-slate-600" /></div>
                  <div className="flex-1">
                    ...
                    <ol className="space-y-1">
                      {r.steps.map((s, j) => <li key={j} className="text-sm text-slate-600 flex items-start gap-2"><span className="w-5 h-5 bg-slate-100 rounded-full flex items-center justify-center flex-shrink-0 text-xs font-bold text-slate-500">{j + 1}</span>{s}</li>)}
                    </ol>
                    {r.warning && <p className="mt-3 text-xs text-amber-700 bg-amber-50 p-2 rounded-lg">{r.warning}</p>}
                  </div>
                </div>
              </div>
            ))}
      ...
      <section className="py-12 bg-slate-50">
        <div className="max-w-4xl mx-auto px-4">
          <h2 className="text-2xl font-bold text-slate-900 mb-6">FAQ - Getting to Monza</h2>
