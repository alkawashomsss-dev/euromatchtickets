      ...
      <section className="relative py-20 overflow-hidden">
        ...
        <div className="relative max-w-6xl mx-auto px-4 text-center">
          ...
          <div className="flex flex-wrap justify-center gap-4 mb-8">
            <div className="flex items-center gap-2 px-4 py-2 bg-slate-50 rounded-full"><Calendar className="w-5 h-5 text-red-600" /><span>June - September 2026</span></div>
            <div className="flex items-center gap-2 px-4 py-2 bg-slate-50 rounded-full"><MapPin className="w-5 h-5 text-red-600" /><span>12 European Cities</span></div>
            <div className="flex items-center gap-2 px-4 py-2 bg-slate-50 rounded-full"><Users className="w-5 h-5 text-red-600" /><span>Stadium Shows</span></div>
          </div>
      ...
      <section className="py-16">
        <div className="max-w-6xl mx-auto px-4">
          ...
          <div className="grid gap-4">
            {shows.map((show, i) => (
              <Link key={i} to="/events?type=concert" className="group flex flex-col md:flex-row md:items-center justify-between bg-white border border-slate-100 hover:border-red-200 rounded-2xl p-6 transition-all">
                <div className="flex items-center gap-4 mb-4 md:mb-0">
                  <div className="w-14 h-14 bg-red-50 rounded-xl flex items-center justify-center"><Music className="w-7 h-7 text-red-600" /></div>
                  <div>
                    <div className="flex items-center gap-2">
                      <h3 className="font-bold group-hover:text-red-600">{show.venue}</h3>
                      {show.hot && <Badge className="bg-orange-50 text-orange-600 text-xs animate-pulse">SELLING FAST</Badge>}
                    </div>
