    ...
    <div className="min-h-screen bg-white" data-testid="german-landing-page">
      ...
      <section className="relative bg-gradient-to-br from-slate-950 via-amber-950 to-slate-950 text-white overflow-hidden">
        <div className="absolute inset-0 opacity-10">
          <div className="absolute top-10 left-10 w-72 h-72 bg-amber-500 rounded-full blur-3xl anim-pulse-slow" />
        ...
        <div className="relative max-w-7xl mx-auto px-4 py-16 md:py-24">
          <div className="inline-flex items-center gap-2 bg-amber-600/20 border border-amber-500/30 rounded-full px-4 py-1.5 text-sm mb-6 anim-fade-in">
            <span className="w-2 h-2 bg-amber-500 rounded-full anim-blink" />
          ...
          <div className="flex flex-wrap gap-4 anim-slide-up" style={{animationDelay: '0.3s'}}>
            <Button size="lg" className="bg-amber-600 hover:bg-amber-700 text-white px-8 py-6 text-lg font-bold rounded-xl" onClick={() => navigate("/events")}>
              Alle Tickets Ansehen <ChevronRight className="ml-2 w-5 h-5" />
            </Button>
            <Button size="lg" variant="outline" className="border-white/30 text-white hover:bg-white/10 px-8 py-6 text-lg rounded-xl" onClick={() => navigate("/bayern-vs-real-madrid-tickets")}>
      ...
      <section className="max-w-7xl mx-auto px-4 py-14">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          {vorteile.map((v, i) => (
            <div key={i} className="text-center p-5 rounded-2xl border border-slate-100 hover:border-amber-200 hover:shadow-lg transition-all anim-fade-in-up" style={{animationDelay: `${i * 0.1}s`}}>
              <v.icon className="w-8 h-8 text-amber-600 mx-auto mb-3" />
              <h3 className="font-bold text-slate-900 mb-1">{v.titel}</h3>
      ...
      <section className="max-w-7xl mx-auto px-4 py-12">
        <h2 className="text-2xl md:text-3xl font-black text-slate-900 mb-2">Beliebteste Events</h2>
        <p className="text-slate-500 mb-8">Die meistgesuchten Tickets in Europa - Preise ab nur €49</p>
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
          {events.map((e, i) => (
            <Link key={i} to={e.link} className="group relative bg-white rounded-2xl border border-slate-100 hover:border-amber-300 hover:shadow-xl transition-all overflow-hidden anim-fade-in-up" style={{animationDelay: `${i * 0.08}s`}}>
              <div className="p-5">
                <div className="flex items-start justify-between mb-3">
                  <span className="inline-flex items-center gap-1 text-[10px] font-bold bg-amber-50 text-amber-700 px-2 py-0.5 rounded-full">{e.tag}</span>
                  <e.icon className="w-5 h-5 text-slate-300 group-hover:text-amber-500 transition-colors" />
                </div>
                <h3 className="font-bold text-slate-900 text-lg mb-2 group-hover:text-amber-600 transition-colors">{e.titel}</h3>
                ...
                <div className="mt-4 flex items-center justify-between">
                  <div>
                    ...
                    <span className="text-2xl font-black text-slate-900 ml-1">€{e.preis}</span>
                  </div>
      ...
      <section className="max-w-4xl mx-auto px-4 py-14">
        <h2 className="text-2xl font-black text-slate-900 mb-4">Tickets Kaufen Online - Die Beste Plattform Europas</h2>
        <div className="prose prose-slate max-w-none text-slate-600 space-y-4">
          ...
          <h3 className="text-xl font-bold text-slate-800">Warum bei EuroMatchTickets kaufen?</h3>
          <ul>
          ...
          <h3 className="text-xl font-bold text-slate-800">Verfügbare Tickets</h3>
          <p>
      ...
      <section className="bg-gradient-to-r from-amber-600 to-amber-700 py-12">
        <div className="max-w-4xl mx-auto px-4 text-center text-white">
          ...
          <Button size="lg" className="bg-white text-amber-700 hover:bg-amber-50 font-bold text-lg px-10 py-6 rounded-xl" onClick={() => navigate("/events")}>
