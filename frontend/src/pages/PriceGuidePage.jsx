    ...
    <div className="min-h-screen bg-zinc-950">
      ...
      <section className="relative pt-20 pb-16 overflow-hidden">
        ...
        <div className="relative max-w-6xl mx-auto px-4 pt-16 text-center">
          ...
          <p className="text-xl text-zinc-400 max-w-2xl mx-auto mb-8">{data.subtitle}</p>
      ...
      <section className="py-4 border-y border-white/5 bg-zinc-900/30">
        <div className="max-w-6xl mx-auto px-4">
          <div className="flex flex-wrap justify-center gap-6 text-sm">
            ...
            <div className="flex items-center gap-2 text-emerald-400"><DollarSign className="w-4 h-4" />0% Buyer Fees</div>
          </div>
        </div>
      </section>
        ...
        <div className="max-w-6xl mx-auto px-4">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {data.categories.map((cat, idx) => (
              <div 
                key={idx}
                className={`bg-zinc-900/50 p-6 rounded-2xl border ${idx === 2 ? 'border-purple-500/50 bg-gradient-to-br from-purple-900/20 to-zinc-900/50' : 'border-zinc-800'}`}
              >
                <div className="text-4xl mb-4">{cat.icon}</div>
                <h3 className="text-xl font-bold mb-2">{cat.name}</h3>
                <div className="text-3xl font-bold text-emerald-400 mb-3">{cat.price}</div>
            ...
            <div>
              ...
              <div className="space-y-4">
                {data.cheapestRaces.map((item, idx) => (
                  <div key={idx} className="bg-zinc-900/50 p-4 rounded-xl border border-zinc-800 flex items-center justify-between">
                    <div>
                    ...
                    <div className="text-xl font-bold text-emerald-400">{item.price}</div>
                  </div>
                ))}
              </div>
            </div>
      ...
      <section className="py-16 bg-zinc-900/30">
        <div className="max-w-3xl mx-auto px-4 text-center">
          ...
          <p className="text-zinc-400 mb-6">Find the best prices on EuroMatchTickets with 0% buyer fees and instant delivery.</p>
          <Link to={data.link}>
            <Button size="lg" className="bg-gradient-to-r from-purple-600 to-emerald-600 hover:from-purple-700 hover:to-emerald-700">
          ...
          <div className="flex flex-wrap justify-center gap-4">
            {Object.keys(PRICE_DATA).filter(type => type !== eventType).map(type => (
              <Link 
                ...
                className="bg-zinc-800/50 hover:bg-zinc-800 border border-zinc-700 hover:border-purple-500/50 px-6 py-3 rounded-full transition-colors capitalize"
              >
