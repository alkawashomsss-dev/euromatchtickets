      ...
      <div className="max-w-[1440px] mx-auto px-4 md:px-8 py-8">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-slate-500 mt-1">Manage revenue, payouts, and sellers</p>
          </div>
          ...
          <div className="bg-white border border-slate-100 rounded-2xl">
            ...
            <div className="divide-y divide-white/5">
              {sellers.length > 0 ? (
                sellers.map((seller) => (
                  <div key={seller.user_id} className="p-4 flex items-center justify-between">
                    ...
                    <div className="text-right">
                      ...
                      <p className="text-2xl font-bold text-emerald-600">€{seller.pending_balance?.toFixed(2)}</p>
                      <p className="text-xs text-slate-400">Total paid: €{seller.total_paid?.toFixed(2)}</p>
                ...
                <div className="p-8 text-center text-slate-400">No sellers registered yet</div>
              )}
            </div>
          </div>
        )}
                ...
                payouts.map((payout) => (
                  <div key={payout.payout_id} className="p-4 flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-sm text-slate-400">{payout.seller_email}</p>
                      <p className="text-xs text-slate-500 mt-1">
                        {new Date(payout.created_at).toLocaleDateString()}
                      </p>
                      {payout.notes && (
                        <p className="text-xs text-slate-400 mt-1">Note: {payout.notes}</p>
                      )}
                    </div>
