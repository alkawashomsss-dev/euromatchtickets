...
const JuventusHubPage = () => {
  ...
  const reviewSchema = {
    ...
    "offers": { "@type": "AggregateOffer", "priceCurrency": "EUR", "lowPrice": "45", "highPrice": "1200", "offerCount": "346" }
  };

  return (
    <div className="min-h-screen bg-[hsl(210,20%,98%)]" data-testid="juventus-hub-page">
        ...
        <div className="relative max-w-6xl mx-auto px-4">
          <div className="flex flex-col lg:flex-row items-center gap-10">
            <div className="flex-1 text-center lg:text-left">
              <div className="inline-flex items-center gap-2 bg-white text-black text-xs font-bold px-4 py-2 rounded-full mb-6"><Trophy className="w-3 h-3" /> JUVENTUS FC</div>
              ...
              <Link to="/events?search=Juventus"><Button size="lg" className="bg-white hover:bg-slate-100 text-black font-bold px-8 rounded-full" data-testid="juventus-cta"><Ticket className="w-5 h-5 mr-2" /> Browse Juventus Matches from &euro;45</Button></Link>
            </div>
            <div className="bg-white rounded-2xl shadow-xl p-6 min-w-[280px]">
              ...
              {[["Capacity", "41,507"], ["Serie A Titles", "36"], ["UCL Finals", "9"], ["Our Prices From", "€45"]].map(([k, v], i) => (
                <div key={i}><div className="flex justify-between py-2"><span className="text-sm text-slate-600">{k}</span><span className="font-bold text-slate-900">{v}</span></div>{i < 3 && <div className="h-px bg-slate-100" />}</div>
              ))}
            </div>
          </div>
        </div>
      ...
      <section className="py-16"><div className="max-w-6xl mx-auto px-4">
        <h2 className="text-2xl font-bold text-slate-900 mb-8">Upcoming Juventus Matches</h2>
        <div className="space-y-3">{matches.map((m, i) => (
          <Link key={i} to={m.link} className="flex flex-col sm:flex-row sm:items-center justify-between bg-white border border-slate-200 hover:border-slate-400 rounded-xl p-5 transition-all" data-testid={`juve-match-${i}`}>
            <div><div className="flex items-center gap-2 mb-1"><h3 className="font-bold text-slate-900">{m.home} vs {m.away}</h3>{m.hot && <span className="text-[10px] bg-slate-900 text-white px-2 py-0.5 rounded-full font-bold">HOT</span>}</div><p className="text-sm text-slate-500">{m.label} &bull; {m.venue} &bull; {m.date}</p></div>
            <div className="flex items-center gap-4 mt-3 sm:mt-0"><span className="text-xs text-slate-500">{m.tickets} left</span><span className="text-xl font-bold text-slate-900">&euro;{m.price}</span><Button className="bg-slate-900 hover:bg-slate-800 text-white">Buy</Button></div>
          </Link>
        ))}</div>
      </div></section>

      {/* FAQ */}
      <section className="py-16 bg-slate-50"><div className="max-w-4xl mx-auto px-4">
        <h2 className="text-2xl font-bold text-slate-900 mb-6">FAQ - Juventus Tickets</h2>
        <FAQStructuredData faqs={faqs} />
        <div className="space-y-3">{faqs.map((f, i) => (
          <details key={i} className="group bg-white rounded-xl border border-slate-200 transition">
      ...
      <section className="py-12"><div className="max-w-5xl mx-auto px-4">
        <h2 className="text-lg font-bold text-slate-900 mb-4">More Teams & Events</h2>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[{ to: "/real-madrid-tickets", label: "Real Madrid", price: "49" }, { to: "/barcelona-tickets", label: "FC Barcelona", price: "45" }, { to: "/bayern-munich-tickets", label: "Bayern Munich", price: "65" }, { to: "/psg-tickets", label: "PSG", price: "55" }, { to: "/liverpool-tickets", label: "Liverpool FC", price: "45" }, { to: "/manchester-city-tickets", label: "Man City", price: "55" }, { to: "/champions-league-tickets", label: "Champions League", price: "49" }, { to: "/f1-italian-grand-prix-monza-tickets", label: "Monza GP", price: "69" }].map((l, i) => (
            <Link key={i} to={l.to} className="bg-white border border-slate-200 rounded-xl p-4 hover:border-slate-400 hover:shadow-md transition-all"><p className="font-bold text-slate-900 text-sm">{l.label}</p><p className="text-emerald-600 text-xs font-bold mt-1">From &euro;{l.price}</p></Link>
          ))}
        </div>
      </div></section>
