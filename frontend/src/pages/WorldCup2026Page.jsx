    ...
    <div className="min-h-screen bg-zinc-950">
      ...
      <div className="fixed top-20 right-4 z-50 flex gap-1 bg-zinc-900/90 rounded-lg p-1 border border-zinc-800">
        {Object.keys(TRANSLATIONS).map(l => (
          <button key={l} onClick={() => setLang(l)} className={`px-2 py-1 rounded text-xs font-bold ${lang === l ? 'bg-amber-600 text-white' : 'text-zinc-400 hover:text-white'}`}>
            {l.toUpperCase()}
          </button>
        ))}
      </div>
      ...
      <section className="relative pt-20 pb-16 overflow-hidden">
        ...
        <div className="relative max-w-6xl mx-auto px-4 pt-20 text-center">
          ...
          <p className="text-xl text-emerald-400 font-semibold mb-4">Official Tickets - 5% Below Market Price</p>
          ...
          {finalEvent && (
            <div className="inline-flex items-center gap-2 bg-gradient-to-r from-amber-500/20 to-yellow-500/20 border border-amber-500/50 px-6 py-3 rounded-full mb-6">
              ...
              <span className="text-amber-400 font-bold">{t.final}: ${finalEvent.lowest_price?.toLocaleString()}</span>
            </div>
          )}
          ...
          <div className="flex flex-wrap justify-center gap-4 mb-8">
            ...
            <div className="flex items-center gap-2 bg-amber-500/20 text-amber-400 px-4 py-2 rounded-full"><Flame className="w-5 h-5" />High Demand</div>
          </div>

          <div className="text-4xl font-bold text-white mb-6">
            {t.from} <span className="text-emerald-400">${events[0]?.lowest_price?.toLocaleString() || '285'}</span>
            <span className="text-lg text-zinc-400 ml-3">to</span>
            <span className="text-amber-400 ml-2">$35,000</span>
            <span className="text-sm text-amber-400 ml-2">Hospitality</span>
          </div>
      ...
      <section className="py-4 border-y border-white/5 bg-zinc-900/50">
        <div className="max-w-6xl mx-auto px-4">
          <div className="flex flex-wrap justify-center gap-8 text-sm">
            <div className="flex items-center gap-2"><Shield className="w-4 h-4 text-emerald-400" />FIFA Official Partner</div>
            <div className="flex items-center gap-2"><Star className="w-4 h-4 text-yellow-400" />4.9/5 Trustpilot</div>
            <div className="flex items-center gap-2"><Users className="w-4 h-4 text-purple-400" />500K+ Tickets Sold</div>
            <div className="flex items-center gap-2"><Globe className="w-4 h-4 text-blue-400" />Worldwide Delivery</div>
          </div>
        </div>
      </section>
      ...
      <section className="py-12 bg-gradient-to-r from-amber-900/20 via-zinc-900 to-yellow-900/20">
        <div className="max-w-6xl mx-auto px-4">
          <h2 className="text-2xl font-bold mb-6 text-center flex items-center justify-center gap-2">
            <Crown className="w-6 h-6 text-amber-400" />Knockout Stage - Premium Tickets
          </h2>
          ...
          <div className="space-y-4">
              ...
              <Link to={`/event/${finalEvent.event_id}`} className="block bg-gradient-to-r from-amber-900/30 to-yellow-900/30 border border-amber-500/50 rounded-xl p-6 hover:border-amber-400 transition-all group">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <Badge className="bg-amber-500 text-white mb-1">FINAL</Badge>
                      <h3 className="text-xl font-bold group-hover:text-amber-400 transition-colors">{finalEvent.title}</h3>
                  ...
                  <div className="flex items-center gap-4">
                    <div className="text-right">
                      ...
                      <div className="text-2xl font-bold text-amber-400">${finalEvent.lowest_price?.toLocaleString()}</div>
                    </div>
            ...
            {semiEvents.map((event) => (
              <Link key={event.event_id} to={`/event/${event.event_id}`} className="block bg-zinc-900/50 border border-purple-500/30 rounded-xl p-6 hover:border-purple-500 transition-all group">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <Badge className="bg-purple-500 text-white mb-1">SEMI-FINAL</Badge>
                  ...
                  <div className="flex items-center gap-4">
                    <div className="text-right">
                      <div className="text-sm text-zinc-400">{t.from}</div>
                      <div className="text-2xl font-bold text-emerald-400">${event.lowest_price?.toLocaleString()}</div>
                    </div>
      ...
      <section className="py-16">
        <div className="max-w-6xl mx-auto px-4">
          ...
          {loading ? (
            ...
            <div className="text-center py-10 text-zinc-400">Loading World Cup matches...</div>
          ) : (
            <div className="grid md:grid-cols-2 gap-4">
              {groupEvents.map((event) => (
                <Link key={event.event_id} to={`/event/${event.event_id}`} className="block bg-zinc-900/50 border border-zinc-800 hover:border-emerald-500/50 rounded-xl p-5 transition-all group">
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex-1">
                      <h3 className="font-bold group-hover:text-emerald-400 transition-colors">{event.title}</h3>
                      <div className="text-sm text-zinc-400">{formatDate(event.event_date)} • {event.city}</div>
                    </div>
                    <div className="text-right">
                      <div className="text-lg font-bold text-emerald-400">${event.lowest_price?.toLocaleString()}</div>
                    </div>
                  </div>
                </Link>
              ))}
