...
const MonzaTicketPricesPage = () => {
  ...
  return (
    <div className="min-h-screen bg-[hsl(210,20%,98%)]" data-testid="monza-prices-page">
      ...
      <section className="py-16">
        <div className="max-w-5xl mx-auto px-4">
          ...
          <h1 className="text-3xl md:text-4xl font-black text-slate-900 mb-4">Monza F1 Ticket Prices 2026 - Complete Comparison</h1>
          <p className="text-lg text-slate-600 mb-10">We compared prices across 4 major platforms. EuroMatchTickets offers the cheapest Monza tickets - save up to 40%.</p>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden mb-12">
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-slate-50 border-b border-slate-200">
                    ...
                    <th className="text-center py-4 px-4 font-bold text-emerald-700 bg-emerald-50">EuroMatch</th>
                ...
                <tbody>
                  {comparison.map((c, i) => (
                    <tr key={i} className="border-b border-slate-100 hover:bg-slate-50">
                      <td className="py-4 px-4 font-medium text-slate-800">{c.category}</td>
                      <td className="py-4 px-4 text-center font-bold text-emerald-700 bg-emerald-50 text-lg">&euro;{c.ours}</td>
            ...
            <div className="p-4 bg-emerald-50 border-t border-emerald-200 text-center">
          ...
          {/* When to Buy */}
          <h2 className="text-2xl font-bold text-slate-900 mb-6 flex items-center gap-2"><TrendingDown className="w-6 h-6 text-emerald-600" /> When to Buy - Price History</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 mb-12">
            {monthlyPrices.map((m, i) => (
              <div key={i} className={`bg-white rounded-xl border p-4 ${i === 0 ? 'border-emerald-300 ring-2 ring-emerald-100' : 'border-slate-200'}`}>
                <p className="font-bold text-slate-900">{m.month}</p>
                <div className="flex gap-4 mt-2">
                  <div><p className="text-[10px] text-slate-400">GA</p><p className="font-bold text-slate-800">&euro;{m.ga}</p></div>
                  <div><p className="text-[10px] text-slate-400">Parabolica</p><p className="font-bold text-slate-800">&euro;{m.parabolica}</p></div>
                </div>
          ...
          <div className="bg-red-600 rounded-2xl p-8 text-center text-white">
            ...
            <Link to="/f1-italian-grand-prix-monza-tickets"><Button size="lg" className="bg-white text-red-600 hover:bg-red-50 font-bold px-8 rounded-full">Buy Monza Tickets Now</Button></Link>
          </div>
        </div>
      </section>
      ...
      <section className="py-12 bg-slate-50">
        <div className="max-w-4xl mx-auto px-4">
          <h2 className="text-2xl font-bold text-slate-900 mb-6">FAQ - Monza Ticket Prices</h2>
      ...
      <section className="py-12">
        <div className="max-w-4xl mx-auto px-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <Link to="/f1-italian-grand-prix-monza-tickets" className="bg-white border border-slate-200 rounded-xl p-4 hover:border-red-300 transition-all"><p className="font-bold text-slate-900 text-sm">Buy Monza Tickets</p><p className="text-xs text-emerald-600">From €69 - Instant delivery</p></Link>
            ...
            <Link to="/monza-f1-travel-tips" className="bg-white border border-slate-200 rounded-xl p-4 hover:border-red-300 transition-all"><p className="font-bold text-slate-900 text-sm">Monza Tips & Travel Guide</p><p className="text-xs text-slate-500">Expert insider tips</p></Link>
          </div>
        </div>
      </section>
    </div>
