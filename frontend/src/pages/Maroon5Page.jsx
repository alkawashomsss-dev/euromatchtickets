    ...
    <div className="min-h-screen bg-zinc-950">
      ...
      <div className="fixed top-20 right-4 z-50 flex gap-1 bg-zinc-900/90 rounded-lg p-1 border border-zinc-800">
        {Object.keys(TRANSLATIONS).map(l => (
          <button
            key={l}
            onClick={() => setLang(l)}
            className={`px-2 py-1 rounded text-xs font-bold ${lang === l ? 'bg-purple-600 text-white' : 'text-zinc-400 hover:text-white'}`}
          >
            {l.toUpperCase()}
          </button>
        ))}
      ...
      <section className="relative pt-20 pb-16 overflow-hidden">
        ...
        <div className="relative max-w-6xl mx-auto px-4 pt-20 text-center">
          ...
          <div className="flex flex-wrap justify-center gap-4 mb-8">
            ...
            <div className="flex items-center gap-2 bg-amber-500/20 text-amber-400 px-4 py-2 rounded-full">
              <Star className="w-5 h-5" />{t.cheapest}
            </div>
          </div>

          <div className="text-4xl font-bold text-white mb-6">
            {t.from} <span className="text-emerald-400">€{events[0]?.lowest_price || 71}</span>
            <span className="text-lg text-zinc-400 line-through ml-3">€{Math.round((events[0]?.lowest_price || 71) / 0.95)}</span>
            <span className="text-sm text-emerald-400 ml-2">-5%</span>
          </div>
            ...
            <div className="space-y-4">
              {events.map((event) => (
                <Link
                  ...
                  className="block bg-zinc-900/50 border border-zinc-800 hover:border-red-500/50 rounded-xl p-6 transition-all group"
                >
        ...
        <div className="max-w-6xl mx-auto px-4">
          ...
          <div className="grid md:grid-cols-4 gap-6">
            {[
              ...
              <div key={idx} className="text-center p-6 bg-zinc-900/50 rounded-2xl border border-zinc-800">
                <div className="w-16 h-16 mx-auto mb-4 rounded-2xl bg-gradient-to-br from-red-600/20 to-pink-600/20 flex items-center justify-center text-red-400">
                  {item.icon}
                </div>
      ...
      <section className="py-16">
        <div className="max-w-3xl mx-auto px-4 text-center">
          ...
          <p className="text-zinc-400 mb-6">Limited tickets available. Book now before they sell out!</p>
