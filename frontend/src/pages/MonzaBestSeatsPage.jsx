...
const MonzaBestSeatsPage = () => {
  ...
  return (
    <div className="min-h-screen bg-[hsl(210,20%,98%)]" data-testid="monza-seats-page">
      ...
      <section className="py-16">
        <div className="max-w-4xl mx-auto px-4">
          ...
          <h1 className="text-3xl md:text-4xl font-black text-slate-900 mb-4">Best Seats at Monza F1 Grand Prix 2026</h1>
          <p className="text-lg text-slate-600 mb-8">Our expert guide to every grandstand at the Autodromo Nazionale di Monza. Find the perfect seat for your budget and viewing preferences.</p>
          ...
          <div className="space-y-6">
            {grandstands.map((g, i) => (
              <div key={i} className={`bg-white rounded-2xl border-l-4 ${g.color} p-6 shadow-sm`} data-testid={`grandstand-${i}`}>
                <div className="flex items-start justify-between gap-4 mb-3">
                  <div>
                    <h2 className="text-xl font-bold text-slate-900">{i + 1}. {g.name}</h2>
                    <p className="text-sm text-slate-500 mt-1"><Eye className="w-3.5 h-3.5 inline mr-1" />{g.view}</p>
                  </div>
                  <div className="text-right flex-shrink-0">
                    <div className="text-2xl font-black text-slate-900">{g.rating}<span className="text-sm text-slate-400 font-normal">/10</span></div>
                    <div className="text-emerald-600 font-bold text-sm">From {g.price}</div>
                  </div>
                </div>
                <p className="text-xs text-amber-700 bg-amber-50 inline-block px-2 py-1 rounded-full mb-3">{g.best}</p>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div>
                    <p className="text-xs text-slate-400 font-bold mb-1">PROS</p>
                    {g.pros.map((p, j) => <p key={j} className="text-sm text-slate-700 flex items-center gap-1"><Check className="w-3 h-3 text-emerald-500" />{p}</p>)}
                  </div>
                  <div>
      ...
      <section className="py-12 bg-slate-50">
        <div className="max-w-4xl mx-auto px-4">
          <h2 className="text-2xl font-bold text-slate-900 mb-6">FAQ - Monza Grandstands</h2>
      ...
      <section className="py-12">
        <div className="max-w-4xl mx-auto px-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <Link to="/f1-italian-grand-prix-monza-tickets" className="bg-white border border-slate-200 rounded-xl p-4 hover:border-red-300 transition-all flex items-center gap-3"><span className="text-2xl">🎟️</span><div><p className="font-bold text-slate-900 text-sm">Buy Monza Tickets</p><p className="text-xs text-emerald-600">From €69</p></div></Link>
            <Link to="/monza-ticket-prices" className="bg-white border border-slate-200 rounded-xl p-4 hover:border-red-300 transition-all flex items-center gap-3"><span className="text-2xl">💰</span><div><p className="font-bold text-slate-900 text-sm">Monza Ticket Prices Compared</p><p className="text-xs text-slate-500">Save up to 40%</p></div></Link>
            <Link to="/how-to-get-to-monza" className="bg-white border border-slate-200 rounded-xl p-4 hover:border-red-300 transition-all flex items-center gap-3"><span className="text-2xl">🚆</span><div><p className="font-bold text-slate-900 text-sm">How to Get to Monza</p><p className="text-xs text-slate-500">From Milan in 45 min</p></div></Link>
            <Link to="/monza-f1-travel-tips" className="bg-white border border-slate-200 rounded-xl p-4 hover:border-red-300 transition-all flex items-center gap-3"><span className="text-2xl">📋</span><div><p className="font-bold text-slate-900 text-sm">Monza F1 Tips & Travel Guide</p><p className="text-xs text-slate-500">Expert insider tips</p></div></Link>
          </div>
        </div>
      </section>
    </div>
